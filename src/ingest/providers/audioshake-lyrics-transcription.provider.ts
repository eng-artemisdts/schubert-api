...
import type {
  ILyricsTranscriptionProvider,
  TranscriptionOptions,
} from '../domain/lyrics-transcription.port';
...
export class AudioshakeLyricsTranscriptionProvider implements ILyricsTranscriptionProvider {
  ...
  > {
    ...
    const base = (
      this.config.get<string>('AUDIOSHAKE_BASE')?.trim() ||
      'https://api.audioshake.ai'
    ).replace(/\/$/, '');
    const model = (
      this.config.get<string>('AUDIOSHAKE_MODEL')?.trim() || 'alignment'
    ).toLowerCase();
    if (!['transcription', 'alignment'].includes(model)) {
      throw new ServiceUnavailableException(
        `AUDIOSHAKE_MODEL inválido: ${model}`,
      );
    }
    ...
    const pollMs = Math.max(
      1000,
      parseInt(
        this.config.get<string>('AUDIOSHAKE_POLL_MS')?.trim() || '5000',
        10,
      ) || 5000,
    );
    ...
    if (!assetId)
      throw new ServiceUnavailableException(
        'AudioShake: resposta sem asset id',
      );
    ...
    const crBody = (await cr.json().catch(() => ({}))) as {
      id?: string;
      message?: string;
    };
    if (!cr.ok) {
    ...
    for (;;) {
      ...
      if (
        statuses.length &&
        statuses.every((s) => s === 'completed' || s === 'error')
      )
        break;
    ...
    if (!t)
      throw new ServiceUnavailableException('AudioShake: sem targets na task');
    if (t.status === 'error') {
      throw new ServiceUnavailableException(
        `AudioShake target erro: ${JSON.stringify(t.error)}`,
      );
    }
    const outputs = (t.output as { format?: string; link?: string }[]) || [];
    const jsonOut = outputs.find((o) => o.format === 'json') || outputs[0];
    if (!jsonOut?.link)
      throw new ServiceUnavailableException('AudioShake: sem link JSON');

    const dl = await fetch(jsonOut.link);
    if (!dl.ok)
      throw new ServiceUnavailableException(
        `AudioShake download: ${dl.status}`,
      );
    const rawJson = await dl.json().catch(() => null);

    const segments = audioshakeJsonToLyricSegments(rawJson, languageLabel);
    this.logger.log(
      `AudioShake: ${segments.length} segmento(s), model=${model}`,
    );
    return segments;
  }
}
